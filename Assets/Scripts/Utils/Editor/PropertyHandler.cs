...
class PropertyHandler
{
	...
	private bool isCurrentlyNested
	{
		get
		{
			return this.m_PropertyDrawer != null && ScriptUtility.DrawerStack.Any<PropertyDrawer>() && this.m_PropertyDrawer == ScriptUtility.DrawerStack.Peek();
		}
	}
	...
	{
		Type drawerTypeForType = ScriptUtility.GetDrawerTypeForType(drawnType);
		if (drawerTypeForType != null)
		{
			if (typeof(PropertyDrawer).IsAssignableFrom(drawerTypeForType))
			{
				if (propertyType == null || !propertyType.IsArrayOrList())
				{
					...
					m_PropertyDrawer.GetType().GetField("m_Attribute").SetValue(m_PropertyDrawer, attribute);
				}
			}
			else if (typeof(DecoratorDrawer).IsAssignableFrom(drawerTypeForType))
			{
				if (field == null || !field.FieldType.IsArrayOrList() || propertyType.IsArrayOrList())
				{
					...
					decoratorDrawer.GetType().GetField("m_Attribute").SetValue(decoratorDrawer, attribute);
	...
	public bool OnGUI(Rect position, FPInfo property, GUIContent label, bool includeChildren)
	{
		...
		if (this.propertyDrawer != null)
		{
			...
			//TypeBuilder tt = new TypeBuilder("GenerateFor"+property.memberType,TypeAttributes.Class, t,null,);
			
			SerializedObject o = new SerializedObject();
			this.propertyDrawer.OnGUI(position, property.Copy(), label ?? EditorGUIUtility.TempContent(property.displayName));
			EditorGUIUtility.labelWidth = labelWidth;
			EditorGUIUtility.fieldWidth = fieldWidth;
			result = false;
		}
...
static class Ext
{
	public static bool IsArrayOrList(this Type t) => t.IsArray || (t.IsGenericType && (t.GetGenericTypeDefinition() == typeof(List<>)));
}